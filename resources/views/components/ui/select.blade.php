...
<div class="w-full">
    ...
    <div class="relative">
        ...
        <select 
            {{ $attributes->merge([
                'class' => 'w-full ' . ($icon ? 'pl-10' : 'px-4') . ' pr-10 py-2.5 rounded-xl border ' . 
                ($error ? 'border-rose-500 ring-1 ring-rose-500' : 'border-slate-200 dark:border-slate-700') . 
                ' bg-slate-50 dark:bg-slate-900/50 text-slate-800 dark:text-slate-200 text-sm focus:outline-none focus:ring-2 ' . 
                ($error ? 'focus:ring-rose-500' : 'focus:ring-indigo-500/50') . 
                ' focus:border-indigo-500 transition-all duration-200 appearance-none'
            ]) }}
        >
            {{ $slot }}
        </select>
