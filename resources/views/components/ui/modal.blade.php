...
<div x-data="{ show: false }" x-on:open-modal.window="if ($event.detail.name === '{{ $name }}') show = true"
    ...
    <template x-teleport="body">
        <div x-show="show" x-on:keydown.escape.window="show = false"
            ...
            <!-- Modal Card -->
            <div class="bg-white dark:bg-slate-900 rounded-3xl shadow-2xl w-full max-w-{{ $maxWidth }} p-6 relative z-10 border border-slate-200 dark:border-slate-700"
                x-transition:enter="transition ease-out duration-300 transform"
                x-transition:enter-start="opacity-0 scale-95 translate-y-4"
                x-transition:enter-end="opacity-100 scale-100 translate-y-0">
                <button x-on:click="show = false"
                    class="absolute top-4 right-4 p-2 rounded-xl hover:bg-slate-100 dark:hover:bg-slate-700 text-slate-400 transition-colors">
