...
<head>
    ...
    @php
        ...
        $priaImage = $storageUrl($pria?->image) ?? 'https://images.unsplash.com/photo-1507003211169-0a1dd7228f2d?w=400&auto=format';
        $wanitaImage = $storageUrl($wanita?->image) ?? 'https://images.unsplash.com/photo-1583939003579-730e3918a45a?w=400&auto=format';
    ...
    <style>
        * { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        body::before {
            ...
            pointer-events: none;
            z-index: 0;
            background-image: radial-gradient(circle at 20% 20%, rgba(228, 194, 134, 0.24) 0 6px, transparent 7px),
                radial-gradient(circle at 80% 35%, rgba(216, 176, 116, 0.18) 0 8px, transparent 9px),
                radial-gradient(circle at 35% 85%, rgba(234, 206, 159, 0.2) 0 7px, transparent 8px);
            background-size: 120px 120px;
            animation: swayBunga 20s infinite alternate ease-in-out;
        }
        @keyframes swayBunga {
            0% { transform: translateY(0) rotate(0deg); opacity: 0.6; }
            100% { transform: translateY(15px) rotate(3deg); opacity: 0.9; }
        }
        .phone-container {
            max-width: 500px;
            margin: 0 auto;
        ...
        .cover-date { font-size: 0.9rem; opacity: 0.9; margin-bottom: 32px; }
        .guest-card {
        ...
        .btn-gold:active { transform: scale(0.96); background: #b28b48; }
        ...
        section.section-visible { transform: translateY(0); opacity: 1; }
        .hero-bg-section {
            background: linear-gradient(rgba(30, 20, 15, 0.65), rgba(20, 12, 8, 0.7)), url('{{ $heroImage }}');
        ...
        .hero-bg-section .quote-block { background: rgba(0,0,0,0.25); backdrop-filter: blur(4px); }
        .section-title {
            font-family: 'Playfair Display', serif;
            font-size: 1.9rem;
            text-align: center;
        ...
        .floral-divider span { display: inline-block; animation: softSpin 6s infinite alternate; }
        @keyframes gentleFloat {
            0%, 100% { transform: translateY(0); opacity: 0.7; }
            50% { transform: translateY(5px); opacity: 1; }
        }
        @keyframes softSpin { 0% { transform: rotate(0deg); } 100% { transform: rotate(8deg); } }
        .couple-names-hero {
            text-align: center;
            font-family: 'Playfair Display', serif;
            font-size: 2.5rem;
        ...
        .hero-date { text-align: center; font-weight: 500; letter-spacing: 1px; }
        .quote-block {
            background: #fef7e8;
            padding: 24px;
            border-radius: 28px;
        ...
        .countdown { display: flex; justify-content: space-between; gap: 12px; margin: 28px 0; }
        .cd-card {
            background: white;
        ...
        .timeline-content strong { font-size: 1.1rem; display: block; color: #9b6a3e; }
        ...
        .wish-card { background: #fefaf5; border-radius: 28px; padding: 16px; margin-bottom: 16px; }
        .wish-alert {
            border-radius: 22px;
            padding: 12px 14px;
            margin-bottom: 14px;
        ...
        .wish-alert.success { background: #eef8ed; border: 1px solid #a9d8a2; }
        .wish-alert.error { background: #fff0ee; border: 1px solid #e7a59b; }
        .floating-music {
            position: fixed;
            bottom: 80px;
            right: 16px;
        ...
        .bottom-nav {
            ...
            z-index: 1100;
            border: 0.5px solid #e9dacb;
        }
        .nav-item { text-align: center; color: #6e4f2e; font-size: 0.7rem; transition: 0.2s; cursor: pointer; }
        ...
        @media (max-width: 500px) {
            .phone-container, .cover-screen { max-width: 100%; }
            .countdown { gap: 8px; }
    ...
    <div class="phone-container">
        ...
        <div class="main-content" id="mainContent">
            <section id="heroSection" class="hero-bg-section" style="padding-top: 56px;">
                <div class="couple-names-hero">{{ $pria?->nama_lengkap ?? $pria?->nama_panggilan ?? 'Mempelai' }} <span style="font-weight:400">&amp;</span> {{ $wanita?->nama_lengkap ?? $wanita?->nama_panggilan ?? 'Mempelai' }}</div>
            ...
            <section id="coupleSection">
                ...
                <div class="couple-card">
                    <img class="couple-img" src="{{ $priaImage }}" alt="{{ $pria?->nama_lengkap ?? 'Mempelai Pria' }}">
                    <h3>{{ $pria?->nama_lengkap ?? $pria?->nama_panggilan ?? 'Mempelai Pria' }}</h3>
                    <p>{{ $pria?->deskripsi ?? 'Mempelai pria' }}</p>
                </div>
                <div class="ampersand">&amp;</div>
                <div class="couple-card">
                    <img class="couple-img" src="{{ $wanitaImage }}" alt="{{ $wanita?->nama_lengkap ?? 'Mempelai Wanita' }}">
            ...
            <section id="eventSection">
                ...
                @foreach ($data->acara as $item)
                    ...
                    <div class="event-card">
                        ...
                        <p><i class="far fa-calendar-alt"></i> {{ Carbon::parse($item->date)->translatedFormat('l, d F Y') }}</p>
                        <p><i class="far fa-clock"></i> {{ $item->jam_start }} {{ $item->zona_waktu }} @if ($item->jam_end === 'Selesai') s/d Selesai @else s/d {{ $item->jam_end }} {{ $item->zona_waktu }} @endif</p>
                        <p><i class="fas fa-map-marker-alt"></i> {{ $item->vanue }}<br><span style="font-size:12px;">{{ $item->alamat }}</span></p>
                        <a href="https://calendar.google.com/calendar/render?action=TEMPLATE&text={{ urlencode($item->nama_acara) }}&dates={{ $startDateTime }}/{{ $endDateTime }}&details={{ urlencode('Jangan lewatkan acara ini') }}&location={{ urlencode($item->alamat) }}" target="_blank" class="btn-outline-gold"><i class="fas fa-calendar-check"></i> Simpan Tanggal</a>
                        @if ($item->maps)
                            <a href="{{ $item->maps }}" target="_blank" class="btn-outline-gold"><i class="fas fa-directions"></i> Petunjuk Lokasi</a>
                        @endif
                    </div>
                @endforeach
            </section>
                ...
                <section id="storySection">
                    ...
                    @foreach ($data->kisah as $kisah)
                        <div class="timeline-item">
                            <img class="timeline-img" src="{{ $storageUrl($kisah->image?->image) ?? $heroImage }}" alt="{{ $kisah->title }}" loading="lazy">
                ...
                <section id="gallerySection">
                    ...
                    @if ($video)
                        <div class="event-card" style="padding: 8px;">
                            <iframe src="{{ $video[0] }}" style="width:100%; height:240px; border:0; border-radius:24px;"></iframe>
                        </div>
                    @endif
                    @if ($poto)
                        <div class="gallery-grid" id="galleryGrid">
                            @foreach ($poto as $po)
                                <img src="{{ $storageUrl($po) }}" class="gallery-img" loading="lazy" alt="Galeri">
                            @endforeach
                        </div>
                    @endif
                </section>
            ...
            @if ($data->streaming?->isActive && $data->streaming?->link)
                <section id="streamingSection">
                    ...
                    <div class="event-card" style="text-align:center">
                        ...
                        <a class="btn-outline-gold" href="{{ $data->streaming->link }}" target="_blank"><i class="fab fa-youtube"></i> Tonton Streaming</a>
                    </div>
                </section>
            @endif
            ...
            @if ($data->fiturKado?->isActive && $data->kado->isNotEmpty())
                <section id="giftSection">
                    ...
                    <div class="floral-divider"><span>*</span><span>+</span><span>*</span></div>
                    <p style="text-align:center; margin-bottom:24px;">Doa restu Anda merupakan hadiah terindah. Tanpa mengurangi rasa hormat, tanda kasih dapat disampaikan melalui:</p>
                    @foreach ($data->kado as $gift)
                        <div class="gift-card">
                            <i class="fas fa-university" style="color:#b47c48"></i>
                            ...
                            @endif
                            @if ($gift->nomorPay)
                                <button class="btn-outline-gold copy-btn" data-account="{{ $gift->nomorPay }}" style="margin-top:12px; display:block;"><i class="far fa-copy"></i> Salin Nomor</button>
                            @endif
                        </div>
                    @endforeach
                </section>
                        ...
                        <div id="wishesList" style="margin-top: 32px;">
                            @forelse ($ucapan as $item)
                                <div class="wish-card">
                                    ...
                                    <span style="background:#f0e0ce; border-radius:20px; padding:2px 10px; font-size:12px;">{{ $item->status }}</span>
                                    <p>{{ $item->ucapan }}</p>
            ...
            <section id="closingSection" style="background: linear-gradient(rgba(0,0,0,0.5), rgba(0,0,0,0.5)), url('{{ $heroImage }}'); background-size: cover; background-position: center; color:white; text-align:center; border-bottom: none;">
                <div class="floral-divider"><span>*</span><span>+</span><span>*</span></div>
        ...
        @if ($data->sound?->isActive && $data->sound?->sound)
            ...
            <audio id="bgMusic" loop preload="auto"><source src="{{ $storageUrl($data->sound->sound) }}" type="audio/mpeg"></audio>
        @endif
                        ...
                        if (wishesList && result.doa) {
                            ...
                            wishesList.insertAdjacentHTML('afterbegin', `
                                <div class="wish-card">
                                    ...
                                    <span style="background:#f0e0ce; border-radius:20px; padding:2px 10px; font-size:12px;">${escapeHtml(result.doa.status)}</span>
