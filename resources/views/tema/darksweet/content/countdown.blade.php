<p class="date countdown text-center font-bold text-white" data-date="{{ $data ? date('Y-m-d', strtotime($data->acara[0]->date)) : '2024-10-10' }}" data-aos="zoom-in-up"
    data-aos-duration="2000">
    <span class="text-lg">{{ $data ? $hari[date('l', strtotime($data->acara[0]->date))] : 'Minggu' }}</span><br>
    <span class="text-2xl font-extrabold">{{ $data ? date('d', strtotime($data->acara[0]->date)) : '10' }}</span>
    <span class="text-xl">•</span>
    <span class="text-2xl font-extrabold">{{ $data ? date('m', strtotime($data->acara[0]->date)) : '10' }}</span>
    <span class="text-xl">•</span>
    <span class="text-2xl font-extrabold">{{ $data ? date('Y', strtotime($data->acara[0]->date)) : '2024' }}</span>
</p>
