...
<section
    class="relative min-h-screen flex flex-row items-stretch justify-between bg-cover bg-center overflow-hidden w-full mt-5"
    id="hero"
    style="background-image: linear-gradient(rgba(0,0,0,0.75), rgba(0,0,0,0.75)), url('{{ asset('tema/darksweet/img/pasangan.jpg') }}'); border-bottom: 1px solid rgba(255,255,255,0.1);">
    ...
    <div
        class="relative z-10 flex-1 flex flex-col items-center justify-center px-3 py-12 text-center overflow-hidden my-auto">
        <!-- Judul Utama -->
        <h1 class="text-4xl md:text-5xl mb-5 font-normal tracking-wide"
            style="font-family: 'Great Vibes', Italiana, cursive; color: #ffffff !important; text-shadow: 2px 2px 4px rgba(0,0,0,0.5);">
            Menghitung Hari
        </h1>
        ...
        @php
            ...
            if (
                str_contains($arTitle, 'Ù') ||
                str_contains($arTitle, 'Ø') ||
                str_contains($arTitle, 'Ã') ||
                str_contains($arTitle, 'Â')
            ) {
        ...
        <!-- Arabic Text -->
        <p class="text-xl md:text-2xl mb-5 font-bold tracking-wide" dir="auto"
            style="font-family: 'Noto Naskh Arabic', 'Amiri', serif; color: #d4af37 !important; text-shadow: 1px 1px 2px rgba(0,0,0,0.8);">
            {{ $arTitle }}
        </p>

        <!-- Quranic Verse -->
        <p class="text-xs leading-relaxed font-light italic opacity-95 mb-6 max-w-[220px] mx-auto break-words"
            style="color: #ffffff !important;">
            {!! nl2br(e($data->qoute?->qoute ?? '')) !!}
        </p>

        <!-- Main Couple Image Portrait -->
        <div class="relative mx-auto w-48 h-64 rounded-2xl overflow-hidden border-3 border-white/20 shadow-2xl mb-6 flex-shrink-0"
            data-purpose="couple-portrait" style="border-width: 3px;">
            <img alt="Couple Portrait" class="w-full h-full object-cover object-center"
                src="{{ asset('storage/' . ($thumbnailWa?->thumbnail ?? '')) }}" />
        ...
        <div class="text-center mb-3">
            <a id="googleCalendarBtn" target="_blank" class="inline-block">
                <button type="button"
                    class="px-5 py-2.5 bg-white hover:bg-gray-200 font-bold rounded-xl shadow-xl text-xs tracking-wider uppercase transition-all"
    ...
    <aside
        class="w-16 flex-shrink-0 backdrop-blur-sm flex px-2 flex-col items-center justify-center space-y-12 py-10 z-30 border-l border-white/5"
        style="background-color: rgba(26, 32, 44, 0.85);" data-purpose="countdown-sidebar">
        <div class="flex flex-col items-center">
            <span class="text-2xl font-bold tracking-widest" id="hari" style="color: #ffffff !important;">00</span>
            <span class="sidebar-text text-[10px] font-semibold tracking-widest uppercase mt-2"
                style="color: #d4af37 !important;">HARI</span>
        </div>
        <div class="flex flex-col items-center">
            <span class="text-2xl font-bold tracking-widest" id="jam" style="color: #ffffff !important;">00</span>
            <span class="sidebar-text text-[10px] font-semibold tracking-widest uppercase mt-2"
                style="color: #d4af37 !important;">JAM</span>
        </div>
        <div class="flex flex-col items-center">
            <span class="text-2xl font-bold tracking-widest" id="menit" style="color: #ffffff !important;">00</span>
            <span class="sidebar-text text-[10px] font-semibold tracking-widest uppercase mt-2"
                style="color: #d4af37 !important;">MENIT</span>
        </div>
        <div class="flex flex-col items-center">
            <span class="text-2xl font-bold tracking-widest" id="detik" style="color: #ffffff !important;">00</span>
            <span class="sidebar-text text-[10px] font-semibold tracking-widest uppercase mt-2"
                style="color: #d4af37 !important;">DETIK</span>
        </div>
    </aside>
    <!-- END: Kolom Kanan -->
