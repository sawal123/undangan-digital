    ...
    @php
        ...
        $coverImage =
            ...
            ((count($poto ?? []) ? $storageUrl($poto[0]) : null) ??
                asset('tema/wedding_blue/assets/hero-couple.png'));
        $heroImage =
        ...
        $priaImage =
            ...
            asset('tema/wedding_blue/assets/gallery-1.png');
        $wanitaImage =
            $storageUrl($wanita?->image) ??
            asset('tema/wedding_blue/assets/gallery-2.png');
            ...
            <div class="cover-inner">
                ...
                <div class="envelope-box">
                    ...
                    <div class="envelope">
                        <div class="back"></div>
                        <div class="liner"></div>
                        <div class="letter"><div><span>You are invited</span><b>{{ $coupleNames }}</b></div></div>
    ...
    <div class="site">
        ...
        <header class="hero" id="home">
            ...
            <div class="hero-copy reveal">
                ...
                <h1>{{ $coupleNames }}</h1>
                <div class="date">{{ $eventDateText }}</div>
                <p>{{ $data->qoute?->qoute ?? 'Two hearts, one promise, and a beautiful journey that leads us to forever.' }}</p>
        ...
        <main>
            ...
            <section id="couple">
                ...
                <div class="section-title reveal">
                    ...
                    <h2>Pengantin</h2>
                    <p>{{ $data->teksUndangan?->acara ?? 'Dengan restu keluarga, kami memohon doa untuk langkah baru kami.' }}</p>
                    <img class="divider" src="{{ asset('tema/wedding_blue/assets/divider.svg') }}" alt="Divider">
                </div>
                <div class="card intro-card glow zoom">
                    ...
                    <div class="names">
                        <div class="person"><div class="label">Mempelai Pria</div><b>{{ $pria?->nama_lengkap ?? 'Mempelai Pria' }}</b><span>{{ $pria?->deskripsi ?? '' }}</span></div>
                        <div class="amp">&</div>
                        <div class="person"><div class="label">Mempelai Wanita</div><b>{{ $wanita?->nama_lengkap ?? 'Mempelai Wanita' }}</b><span>{{ $wanita?->deskripsi ?? '' }}</span></div>
                    </div>
                </div>
            </section>
                    ...
                    <div class="story-list">
                        @foreach ($data->kisah as $kisah)
                            <article class="card story-item {{ $loop->even ? 'reveal-left' : 'reveal-right' }}">
                                <img src="{{ $storageUrl($kisah->image?->image) ?? $heroImage }}" alt="{{ $kisah->title }}">
                                <div><div class="year">{{ $kisah->created_at?->format('Y') ?? 'Kisah' }}</div><h4>{{ $kisah->title }}</h4><p>{{ $kisah->deskripsi }}</p></div>
                            </article>
                        @endforeach
                    </div>
                </section>
            ...
            @if ($data->FiturUcapan?->isActive)
                <section class="soft" id="rsvp">
                    ...
                    <div class="card rsvp reveal">
                        ...
                        @else
                            <p style="color:var(--muted)">Form ucapan hanya tersedia untuk tamu yang menerima tautan undangan.</p>
                        @endif
                    </div>
                        ...
                        <div class="messages" id="wishList">
                            @forelse ($ucapan as $item)
                                <article class="card message reveal">
                                    ...
                                    <p>{{ $item->ucapan }}</p>
                                    @if ($item->balas)
                                        <div style="margin-top:10px; background:#f6f9ff; border-radius:16px; padding:10px; font-size:0.9rem;"><strong>Balasan:</strong> {{ $item->balas }}</div>
                                    @endif
                                </article>
                            @empty
            ...
            <section id="share">
                <div class="card share zoom">
                    ...
                    <div class="share-actions">
                        ...
                        <a class="btn" href="https://wa.me/?text={{ urlencode($pageTitle . ' - ' . url()->current()) }}" target="_blank" rel="noopener">WhatsApp</a>
                    </div>
                </div>
            </section>
            ...
            <section class="closing" id="closing">
                <div class="reveal">
                    ...
                    <h2>With Love,</h2>
                    <p>{!! nl2br(e($data->teksUndangan?->penutup ?? 'Merupakan suatu kehormatan dan kebahagiaan bagi kami apabila Anda berkenan hadir dan memberikan doa restu.')) !!}</p>
