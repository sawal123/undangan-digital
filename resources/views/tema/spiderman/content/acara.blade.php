<div class="relative min-h-screen hidden " id="acara">
    ...
    <div class="w-full flex flex-col items-center justify-center space-y-4 uppercase z-50">
        ...
        <div class="flex flex-row justify-center items-center text-[#FFC300] space-x-1" data-aos="fade-down"
            ...
            <div class="text-[50px] font-semibold  leading-none ">
                {{ $acara?->date ? date('d', strtotime($acara->date)) : '10' }}</div>
            <div class=" text-[20px] font-semibold  leading-none text-start">
                {{ $acara?->date ? date('m', strtotime($acara->date)) : '10' }}<br>{{ $acara?->date ? date('Y', strtotime($acara->date)) : '2024' }}
            </div>

            <!-- Divider -->
            <div class="h-16 w-[2px] bg-[#FFC300]"></div>

            <div class=" text-[20px] font-semibold  leading-none text-end">{{ $acara?->jam_start ?? '17.00' }}
                {{ $acara?->zona_waktu ?? '' }}<br>S.D {{ $acara?->jam_end ?? 'SELESAI' }}</div>
        </div>
        ...
        <div class="text-center pt-1 " data-aos="fade-down" data-aos-duration="2000" data-aos-easing="linear">
            <a href="{{ $acara?->maps ?? '#' }}" target="_blank"
        ...
        <div class="text-center pt-1 z-50" data-aos="fade-up" data-aos-duration="2500" data-aos-easing="linear">
            ...
            <script>
                document.addEventListener("DOMContentLoaded", function() {
                    ...
                    let eventDateStart =
                    "{{ $acara?->date ? date('Ymd', strtotime($acara->date)) : date('Ymd') }}T100000Z"; // Sesuaikan jam UTC
        ...
        <script>
            document.addEventListener("DOMContentLoaded", function() {
                ...
                let eventDate = new Date(
                    "{{ $acara?->date ? date('Y-m-d', strtotime($acara->date)) : date('Y-m-d') }}").getTime();
