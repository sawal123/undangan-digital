    ...
    <div class="flex flex-col justify-center ">
        ...
        <div class="text-center max-w-2xl  p-8 text-[12px] md:text-lg text-[#755f4B]">
            <img src="{{ asset('storage/' . ($data->pria?->image ?? '')) }}"
                ...
                alt="{{ $data->pria?->nama_lengkap ?? 'Mempelai Pria' }}" data-aos="fade-up" data-aos-duration="1500">
            ...
            <img src="{{ asset('storage/' . ($data->wanita?->image ?? '')) }}"
                class="w-40 h-40 mx-auto mb-4 rounded-full object-cover"
                alt="{{ $data->wanita?->nama_lengkap ?? 'Mempelai Wanita' }}" data-aos="fade-up"
                data-aos-duration="1500">
            <p class="text-5xl
title mb-2 font-bold " data-aos="fade-up" data-aos-duration="1500">
                {{ $data->wanita?->nama_lengkap ?? 'Ajeng Febian' }}</p>
            <p class="text-sm mb-4" data-aos="fade-up" data-aos-duration="1500">
                {{ $data->wanita?->deskripsi ?? 'Putri ke-2 Bpk. Samuel & Ibu Masuya' }}</p>
            @php
                use Carbon\Carbon;

                $acaraOpening = $data->acara?->first();
                $tanggalAcara = $acaraOpening?->date ? Carbon::parse($acaraOpening->date) : null;
            ...
            <a href="https://www.google.com/calendar/render?action=TEMPLATE&text={{ $data->title }}&dates{{ $acaraOpening?->date ?? '' }}&location={{ $acaraOpening?->alamat ?? '' }}"
                target="_blank"
