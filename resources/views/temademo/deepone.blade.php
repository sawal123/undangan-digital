...
<head>
    ...
    <title>Undangan Pernikahan | Melissa & Damian</title>
    ...
    <style>
        * {
        ...
        /* Latar belakang bunga bergoyang (revisi: background bunga halus) */
        body::before {
            content: "";
            position: fixed;
            top: 0;
            ...
            pointer-events: none;
            z-index: 0;
            background-image: url('data:image/svg+xml;utf8,<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 200 200" opacity="0.15"><path fill="%23e4c286" d="M50,40 L55,55 L70,58 L55,62 L50,78 L45,62 L30,58 L45,55 Z" transform="scale(0.8) translate(10,10)"/><path fill="%23d8b074" d="M130,90 L135,105 L150,108 L135,112 L130,128 L125,112 L110,108 L125,105 Z" transform="scale(0.9) translate(5,5)"/><path fill="%23eace9f" d="M80,150 L85,165 L100,168 L85,172 L80,188 L75,172 L60,168 L75,165 Z" transform="scale(0.7) translate(20,10)"/><path fill="%23f0dbaa" d="M170,30 L175,45 L190,48 L175,52 L170,68 L165,52 L150,48 L165,45 Z" transform="scale(0.6) translate(30,60)"/><circle cx="40" cy="120" r="6" fill="%23edcf96" opacity="0.5"/><circle cx="160" cy="180" r="8" fill="%23e7c68a" opacity="0.4"/></svg>');
        ...
        /* desktop container: phone-like center */
        .phone-container {
            max-width: 500px;
            margin: 0 auto;
            background: #fffdf9;
        ...
        .hero-bg-section {
            background: linear-gradient(rgba(30, 20, 15, 0.65), rgba(20, 12, 8, 0.7)), url('https://images.unsplash.com/photo-1519741497674-611481863552?w=800&auto=format');
        ...
        /* timeline dengan thumbnail gambar */
        .timeline-item {
            display: flex;
            gap: 16px;
            margin-bottom: 32px;
        ...
        @media (max-width: 480px) {
            ...
            .timeline-img { width: 60px; height: 60px; }
    ...
    <div class="main-content" id="mainContent">
        ...
        <section id="coupleSection">
            ...
            <div class="couple-card">
                ...
                <a href="#" style="color:#b47c48;"><i class="fab fa-instagram"></i> @melissa.aurel</a>
            </div>
            ...
            <div class="couple-card">
                ...
                <a href="#" style="color:#b47c48;"><i class="fab fa-instagram"></i> @damian.prtma</a>
            </div>
        </section>
        ...
        <!-- Love Story (dengan thumbnail setiap cerita) -->
        ...
        <section id="streamingSection" style="display: none;">
            ...
            <div class="event-card" style="text-align:center">
                ...
                <button class="btn-outline-gold" id="streamBtn"><i class="fab fa-youtube"></i> Tonton Streaming</button>
            </div>
        </section>
        ...
        <section id="rsvpSection">
            ...
            <button id="loadMoreWishes" class="btn-outline-gold" style="width:100%; text-align:center;">Lihat Lebih Banyak</button>
        </section>

        <!-- Closing -->
        <section id="closingSection" style="background: linear-gradient(rgba(0,0,0,0.5), rgba(0,0,0,0.5)), url('https://images.unsplash.com/photo-1519741497674-611481863552?w=800&auto=format'); background-size: cover; background-position: center; color:white; text-align:center; border-bottom: none;">
            <div class="floral-divider"><span>🌙</span> <span>✨</span> <span>🌹</span></div>
...
<script>
    // DATA KONFIGURASI lengkap dengan thumbnail gambar untuk setiap love story
    const config = {
    ...
    function renderEvents() {
        ...
        container.innerHTML = config.events.map(ev => `
            <div class="event-card">
                ...
                <p><i class="far fa-calendar-alt event-icon"></i> ${ev.date}</p>
                <p><i class="far fa-clock event-icon"></i> ${ev.time}</p>
                <p><i class="fas fa-map-marker-alt event-icon"></i> ${ev.location}<br/><span style="font-size:12px;">${ev.address}</span></p>
                <button class="btn-outline-gold map-btn" data-map="${ev.mapLink}"><i class="fas fa-directions"></i> Petunjuk Lokasi</button>
            </div>
        `).join('');
        document.querySelectorAll('.map-btn').forEach(btn => btn.addEventListener('click', (e) => window.open(btn.dataset.map, '_blank')));
    }

    // Render LOVE STORY dengan thumbnail gambar (revisi)
    function renderLoveStory() {
        const container = document.getElementById('timelineContainer');
        container.innerHTML = config.loveStory.map(story => `
            <div class="timeline-item">
                <img class="timeline-img" src="${story.img || 'https://placehold.co/80x80/f6e9d8/c9a46b?text=Kisah'}" alt="story thumbnail" loading="lazy">
                <div class="timeline-year">${story.year}</div>
                <div class="timeline-dot"></div>
                <div class="timeline-content">
                    <strong>${story.title}</strong>
                    <p style="margin-top:4px; font-size:0.9rem;">${story.desc}</p>
                </div>
            </div>
        `).join('');
    }
    ...
    function renderGallery() {
        ...
        grid.innerHTML = config.gallery.map((src,idx) => `<img src="${src}" data-index="${idx}" class="gallery-img" loading="lazy">`).join('');
    ...
    function renderGiftCards() {
        ...
        container.innerHTML = config.giftMethods.map(g => `
            <div class="gift-card">
                <i class="fas fa-university" style="color:#b47c48"></i> <strong>${g.bank}</strong><br/>
                ${g.account}<br/> a.n ${g.holder}
                <button class="btn-outline-gold copy-btn" data-account="${g.account}" style="margin-top:12px; display:block;"><i class="far fa-copy"></i> Salin Nomor</button>
            </div>
        `).join('');
    ...
    function renderWishes() {
        ...
        container.innerHTML = showWishes.map(w => `<div class="wish-card"><strong>${w.name}</strong> <span style="background:#f0e0ce; border-radius:20px; padding:2px 10px; font-size:12px;">${w.status}</span><p>${w.message}</p><small>${w.date || "Baru saja"}</small></div>`).join('');
    ...
    document.querySelectorAll('.nav-item').forEach(item => item.addEventListener('click', () => document.getElementById(item.dataset.target)?.scrollIntoView({ behavior: 'smooth', block: 'start' })));
    ...
    const observer = new IntersectionObserver((entries) => { entries.forEach(entry => { if(entry.isIntersecting) entry.target.classList.add('section-visible'); }); }, { threshold: 0.2 });
    sections.forEach(section => observer.observe(section));
    renderEvents(); renderLoveStory(); renderGallery(); renderGiftCards(); renderWishes();
    const urlGuest = new URLSearchParams(window.location.search).get('to');
    if(urlGuest) document.getElementById('guestNameDisplay').innerText = decodeURIComponent(urlGuest);
</script>
</body>
</html>