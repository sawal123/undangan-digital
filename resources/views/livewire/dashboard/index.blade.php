<div class="space-y-6">
    <!-- Top Action Bar -->
    <div
        class="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 bg-white dark:bg-slate-900 p-5 rounded-2xl border border-slate-200 dark:border-slate-800 shadow-sm">
        <div>
        ...
        <a href="{{ url('dashboard/setup') }}" wire:navigate
            class="inline-flex items-center justify-center px-5 py-2.5 bg-indigo-600 hover:bg-indigo-700 text-white text-sm font-semibold rounded-xl transition-all gap-2 shadow-sm shadow-indigo-200 dark:shadow-none">
    ...
    <div class="grid grid-cols-1 gap-4">
        @forelse ($dataUndangan as $item)
            ...
            @endphp
            <div
                class="bg-white dark:bg-slate-900 p-5 rounded-2xl border border-slate-200 dark:border-slate-800 shadow-sm hover:shadow-md transition-all group">
                <div class="flex flex-col lg:flex-row lg:items-center justify-between gap-6">
                    <!-- Title & Info -->
                    <div class="flex items-center gap-4 min-w-0">
                        <div
                            class="w-12 h-12 rounded-xl bg-indigo-50 dark:bg-indigo-900/30 flex items-center justify-center flex-shrink-0 group-hover:scale-110 transition-transform">
                            <i data-lucide="mail" class="w-6 h-6 text-indigo-600 dark:text-indigo-400"></i>
                        </div>
                        <div class="min-w-0">
                            <h4 class="text-md font-bold text-slate-800 dark:text-white truncate">{{ $item->title }}
                            </h4>
                            <div class="flex items-center gap-3 mt-1">
                                ...
                                <span
                                    class="px-2 py-0.5 rounded-full bg-indigo-50 dark:bg-indigo-900/30 text-indigo-600 dark:text-indigo-400 text-[10px] font-bold uppercase tracking-wider">
                                    {{ $item->eventType->name ?? 'Pernikahan' }}
                                </span>
                                @if ($canShareInvitation)
                                    <span
                                        class="px-2 py-0.5 rounded-full bg-amber-50 dark:bg-amber-900/30 text-amber-600 dark:text-amber-400 text-[10px] font-bold uppercase tracking-wider flex items-center gap-1">
                                        <i data-lucide="crown" class="w-3 h-3"></i> Premium
                                    </span>
                                @else
                                    <span
                                        class="px-2 py-0.5 rounded-full bg-slate-100 dark:bg-slate-700 text-slate-500 dark:text-slate-400 text-[10px] font-bold uppercase tracking-wider">Free</span>
                                @endif
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="flex flex-wrap items-center gap-2">
                        ...
                        @else
                            <span
                                class="inline-flex items-center px-4 py-2 bg-slate-100 dark:bg-slate-800 text-slate-400 text-sm font-medium rounded-xl cursor-not-allowed">
        ...
        @empty
            <div
                class="py-20 flex flex-col items-center justify-center text-slate-500 dark:text-slate-400 bg-white dark:bg-slate-900 rounded-3xl border-2 border-dashed border-slate-200 dark:border-slate-800">
                <div
                    class="w-20 h-20 bg-slate-50 dark:bg-slate-950/50 rounded-full flex items-center justify-center mb-4">
                ...
                <a href="{{ url('dashboard/setup') }}" wire:navigate
                    class="px-6 py-3 bg-indigo-600 hover:bg-indigo-700 text-white font-semibold rounded-2xl transition-all shadow-lg shadow-indigo-200 dark:shadow-none">
