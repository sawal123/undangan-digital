<div class="space-y-6" x-data="{ deleteId: null, deleteMethod: 'delete' }"
    ...
    <!-- Header Section -->
    <div
        class="bg-white dark:bg-slate-900 p-5 rounded-2xl border border-slate-200 dark:border-slate-800 shadow-sm flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div>
            <h3 class="text-lg font-bold text-slate-800 dark:text-white">Daftar Tamu Undangan</h3>
            <p class="text-sm text-slate-500 dark:text-slate-400">Kelola dan kirimkan undangan kepada tamu spesial Anda.
            </p>
        </div>
        <button x-on:click="$wire.call('resetField'); $dispatch('open-modal', { name: 'tamu-modal' })"
            class="inline-flex items-center justify-center px-4 py-2.5 bg-indigo-600 hover:bg-indigo-700 text-white text-sm font-semibold rounded-xl transition-all gap-2 shadow-sm shadow-indigo-200 dark:shadow-none">
    ...
    @unless ($canShareInvitation)
        <div
            class="p-4 rounded-2xl bg-amber-50 dark:bg-amber-900/20 border border-amber-200 dark:border-amber-800 text-amber-800 dark:text-amber-300 text-sm flex items-start gap-3">
            <i data-lucide="lock" class="w-5 h-5 mt-0.5"></i>
            <div>
                <p class="font-bold">Link undangan belum bisa dibagikan ke publik.</p>
                <p class="mt-1 text-amber-700 dark:text-amber-400">Upgrade ke premium untuk membagikan link undangan melalui
                    WhatsApp atau copy link ke tamu anda.</p>
            </div>
        </div>
    @endunless
    ...
    <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
        <div class="md:col-span-2 relative" wire:ignore.self>
            ...
            <input type="text" wire:model.live.debounce.500ms="query"
                placeholder="Cari berdasarkan nama, kode, atau nomor WhatsApp..."
        ...
        <div
            class="bg-indigo-50 dark:bg-indigo-900/20 border border-indigo-100 dark:border-indigo-800 rounded-2xl p-3 flex items-center justify-center gap-3">
            <div class="w-10 h-10 rounded-xl bg-indigo-600 flex items-center justify-center text-white">
                <i data-lucide="users" class="w-5 h-5"></i>
            </div>
            <div>
                <p class="text-[10px] font-bold text-indigo-600 dark:text-indigo-400 uppercase tracking-widest">Total
                    Tamu</p>
                        ...
                        <tr class="hover:bg-slate-50 dark:hover:bg-slate-900/30 transition-colors">
                            <td class="px-6 py-4">
                                <div class="flex items-center gap-3">
                                    ...
                                    <div>
                                        <p class="text-sm font-bold text-slate-800 dark:text-white">{{ $item->nama }}
                                        </p>
                                        <p
                                            class="text-[10px] text-slate-500 dark:text-slate-400 uppercase font-bold tracking-tighter">
                                            Kode: {{ $item->kode }}</p>
                                    </div>
                                </div>
                            </td>
                            <td class="px-6 py-4">
                                <span
                                    class="inline-flex items-center gap-1.5 text-xs text-slate-600 dark:text-slate-400 bg-slate-100 dark:bg-slate-700 px-2 py-1 rounded-md">
                            ...
                            <td class="px-6 py-4">
                                <div class="flex items-center justify-center gap-2">
                                    <button wire:click="shareWA({{ $item->id }})" @disabled(!$canShareInvitation)
                                        class="p-2 text-emerald-600 hover:bg-emerald-50 dark:hover:bg-emerald-900/20 rounded-lg transition-colors disabled:text-slate-300 disabled:cursor-not-allowed disabled:hover:bg-transparent"
                                        title="{{ $canShareInvitation ? 'Kirim via WhatsApp' : 'Undangan belum aktif' }}">
                                        <i data-lucide="send" class="w-5 h-5"></i>
                                    </button>
                                    <button wire:click="shareTamu({{ $item->id }})" @disabled(!$canShareInvitation)
                                        class="p-2 text-indigo-600 hover:bg-indigo-50 dark:hover:bg-indigo-900/20 rounded-lg transition-colors disabled:text-slate-300 disabled:cursor-not-allowed disabled:hover:bg-transparent"
                                        title="{{ $canShareInvitation ? 'Salin Link' : 'Undangan belum aktif' }}">
                                        <i data-lucide="copy" class="w-5 h-5"></i>
                                    </button>
                                    <button wire:click="EditTamu({{ $item->id }})" wire:loading.attr="disabled"
                                        wire:target="EditTamu({{ $item->id }})"
                                        class="p-2 text-amber-600 hover:bg-amber-50 dark:hover:bg-amber-900/20 rounded-lg transition-colors disabled:opacity-60 disabled:cursor-not-allowed"
                                        title="Edit">
                                        <i wire:loading.remove wire:target="EditTamu({{ $item->id }})"
                                            data-lucide="edit-3" class="w-5 h-5"></i>
                                        <i wire:loading wire:target="EditTamu({{ $item->id }})"
                                            data-lucide="loader-2" class="w-5 h-5 animate-spin"></i>
                                    </button>
                                    <button
                                        x-on:click="$dispatch('set-delete', { id: '{{ $item->kode }}', method: 'delete' }); $dispatch('open-modal', { name: 'delete-modal' })"
    ...
    <x-ui.modal name="tamu-modal" :title="$idTamu ? 'Edit Tamu' : 'Tambah Tamu'" icon="user">
        <form wire:submit="save" class="space-y-4">
            ...
            <x-ui.input label="WhatsApp (Opsional)" type="number" wire:model="whatsapp"
                placeholder="Contoh: 08123456789" />

            <div class="flex justify-end gap-2 mt-6">
                <x-ui.button variant="secondary" type="button"
                    x-on:click="$dispatch('close-modal', { name: 'tamu-modal' })">Batal</x-ui.button>
                <x-ui.button variant="primary" type="submit" loading-target="save"
                    loading-text="Menyimpan...">Simpan</x-ui.button>
            </div>
        </form>
    </x-ui.modal>
    ...
    <x-ui.modal name="share-modal" title="Bagikan Link Undangan" icon="share-2">
        <div class="space-y-4">
            <div
                class="p-4 bg-indigo-50 dark:bg-indigo-900/20 rounded-2xl border border-indigo-100 dark:border-indigo-800/50">
            ...
            <div class="flex justify-end mt-6">
                <x-ui.button variant="secondary"
                    x-on:click="$dispatch('close-modal', { name: 'share-modal' })">Tutup</x-ui.button>
            </div>
        </div>
    </x-ui.modal>
