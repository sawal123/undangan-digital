<div class="space-y-6">
    ...
    <div class="bg-white dark:bg-slate-900 p-6 rounded-3xl border border-slate-200 dark:border-slate-800 shadow-sm">
        <div class="flex flex-col md:flex-row justify-between items-start md:items-center gap-6">
            <div>
                ...
                <p class="text-sm text-slate-500 dark:text-slate-400">Daftar kehadiran dan pesan dari tamu yang hadir.</p>
            </div>
    ...
    <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
        <div class="bg-white dark:bg-slate-900 p-6 rounded-3xl border border-slate-200 dark:border-slate-800 shadow-sm flex items-center gap-4">
            ...
            <div>
                ...
                <p class="text-2xl font-black text-slate-800 dark:text-white">{{ \App\Models\KelolaUndangan\Ucapan::where('data_id', $dataId)->where('status', 'Hadir')->count() }}</p>
            </div>
        </div>
        <div class="bg-white dark:bg-slate-900 p-6 rounded-3xl border border-slate-200 dark:border-slate-800 shadow-sm flex items-center gap-4">
            ...
            <div>
                ...
                <p class="text-2xl font-black text-slate-800 dark:text-white">{{ \App\Models\KelolaUndangan\Ucapan::where('data_id', $dataId)->where('status', 'Tidak Hadir')->count() }}</p>
            </div>
        </div>
        <div class="bg-white dark:bg-slate-900 p-6 rounded-3xl border border-slate-200 dark:border-slate-800 shadow-sm flex items-center gap-4">
    ...
    <!-- Attendance Table -->
    <div class="bg-white dark:bg-slate-900 rounded-3xl border border-slate-200 dark:border-slate-800 shadow-sm overflow-hidden relative">
        <div
            wire:loading.flex
        ...
        <div class="overflow-x-auto" wire:loading.class="opacity-50 pointer-events-none" wire:target="search,gotoPage,nextPage,previousPage">
            <table class="w-full text-left border-collapse">
                <thead>
                    <tr class="bg-slate-50 dark:bg-slate-900/50 border-b border-slate-100 dark:border-slate-800">
                        ...
                        <th class="px-6 py-4 text-[10px] font-bold text-slate-400 uppercase tracking-widest text-right">Waktu</th>
                    </tr>
                </thead>
                <tbody class="divide-y divide-slate-50 dark:divide-slate-800">
                    @forelse($data as $item)
                        <tr class="hover:bg-slate-50 dark:hover:bg-slate-950/50 transition-colors">
                            <td class="px-6 py-4">
                                <p class="text-sm font-bold text-slate-800 dark:text-white">{{ $item->tamu->nama ?? 'Anonim' }}</p>
                                <p class="text-[10px] text-slate-400 font-medium">ID: #{{ $item->tamu->kode ?? '-' }}</p>
                            </td>
                            <td class="px-6 py-4">
                                ...
                                @elseif($item->status == 'Akan Hadir')
                                    <span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-[10px] font-black uppercase bg-indigo-100 text-indigo-700 dark:bg-indigo-900/30 dark:text-indigo-400">
                                        {{ $item->status }}
                                    </span>
                                @else
                                    <span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-[10px] font-black uppercase bg-slate-100 text-slate-600 dark:bg-slate-800 dark:text-slate-400">
                                        {{ $item->status }}
                                    </span>
                                @endif
                            </td>
                            <td class="px-6 py-4 max-w-xs">
                                <p class="text-sm text-slate-600 dark:text-slate-400 italic line-clamp-2">"{{ $item->ucapan }}"</p>
                            </td>
