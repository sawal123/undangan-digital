<div class="space-y-6">
    ...
    @unless ($canShareInvitation)
        <div
            class="p-4 rounded-2xl bg-blue-50 dark:bg-blue-900/20 border border-blue-200 dark:border-blue-800 text-blue-800 dark:text-blue-300 text-sm flex items-start gap-3">
            <i data-lucide="zap" class="w-5 h-5 mt-0.5"></i>
            <div>
                <p class="font-bold">Link undangan belum bisa dibagikan ke publik.</p>
                <p class="mt-1 text-blue-700 dark:text-blue-400">Upgrade ke premium untuk membagikan link undangan ke tamu
                    anda. Namun, anda tetap dapat melihat preview dengan data anda sendiri.</p>
            </div>
        </div>
    @endunless
                ...
                <div class="aspect-[3/4] overflow-hidden relative bg-slate-100 dark:bg-slate-800">
                    ...
                    <div
                        ...
                        <div class="flex gap-2">
                            <a href="{{ $this->previewUrl($item->path) }}" target="_blank"
                    ...
                    <div class="flex mt-2 flex-col sm:flex-row items-end sm:items-center gap-2">
                        @if ($isSelected)
                            <button type="button" wire:click="review" wire:loading.attr="disabled" wire:target="review"
                                ...
                                <i wire:loading.remove wire:target="review"
                                    data-lucide="{{ $canShareInvitation ? 'eye' : 'lock' }}" class="w-4 h-4"></i>
                                <i wire:loading wire:target="review" data-lucide="loader-2"
                        ...
                        @endif
                        <button wire:click="choose({{ $item->id }})" wire:loading.attr="disabled"
                            wire:target="choose({{ $item->id }})"
                            ...
                            <span wire:loading.remove
                                wire:target="choose({{ $item->id }})">{{ $isSelected ? 'Terpilih' : 'Pilih' }}</span>
                            <span wire:loading.flex wire:target="choose({{ $item->id }})"
                                class="hidden items-center gap-1.5"><i data-lucide="loader-2"
                                    class="w-3.5 h-3.5 animate-spin"></i> Memilih...</span>
                        </button>
                    </div>
                </div>
            </div>
