<div class="space-y-6" x-data="{ deleteId: null, deleteMethod: 'delete' }"
    ...
    <div class="bg-white dark:bg-slate-900 p-6 rounded-3xl border border-slate-200 dark:border-slate-800 shadow-sm">
        <div class="flex flex-col md:flex-row justify-between items-start md:items-center gap-6">
            <div>
                ...
                <p class="text-sm text-slate-500 dark:text-slate-400">Kelola pesan manis dan doa restu dari para tamu
                    undangan Anda.</p>
            </div>

            <div
                class="flex flex-wrap items-center gap-4 bg-slate-50 dark:bg-slate-900/50 p-4 rounded-2xl border border-slate-100 dark:border-slate-800">
                <div class="flex items-center gap-3 pr-4 border-r border-slate-200 dark:border-slate-800">
                    ...
                    <label class="relative inline-flex items-center cursor-pointer">
                        <input type="checkbox"
                            wire:click="updateFiturUcapan('{{ $dataId }}', {{ $fitUcapan && $fitUcapan->isActive ? 'false' : 'true' }}, 'isActive')"
                            class="sr-only peer" @checked($fitUcapan && $fitUcapan->isActive)>
                        <div
                ...
                <div class="flex items-center gap-3">
                    <span class="text-xs font-bold text-slate-500 uppercase tracking-wider">Tampilkan di
                        Undangan:</span>
                    <label class="relative inline-flex items-center cursor-pointer">
                        <input type="checkbox"
                            wire:click="updateFiturUcapan('{{ $dataId }}', {{ $fitUcapan && $fitUcapan->viewIsActive ? 'false' : 'true' }}, 'viewIsActive')"
                            class="sr-only peer" @checked($fitUcapan && $fitUcapan->viewIsActive)>
                        <div
                            class="w-10 h-5 bg-slate-200 peer-focus:outline-none rounded-full peer dark:bg-slate-700 peer-checked:after:translate-x-full peer-checked:after:border-white after:content-[''] after:absolute after:top-[2px] after:left-[2px] after:bg-white after:border-gray-300 after:border after:rounded-full after:h-4 after:w-4 after:transition-all dark:border-gray-600 peer-checked:bg-indigo-600">
                        </div>
                    </label>
    ...
    <div class="grid grid-cols-1 gap-4">
        @forelse($ucapan as $item)
            <div
                class="bg-white dark:bg-slate-900 p-6 rounded-3xl border border-slate-200 dark:border-slate-800 shadow-sm hover:shadow-md transition-all">
                <div class="flex flex-col md:flex-row gap-6">
                    <!-- User Info -->
                    <div class="flex-shrink-0 flex md:flex-col items-center md:items-start gap-4 md:w-48">
                        <div
                            class="w-12 h-12 rounded-2xl bg-indigo-50 dark:bg-indigo-900/30 flex items-center justify-center text-indigo-600 dark:text-indigo-400 font-black">
                            {{ substr($item->tamu->nama ?? 'A', 0, 1) }}
                        </div>
                        <div class="min-w-0">
                            <h4 class="text-sm font-bold text-slate-800 dark:text-white truncate">
                                {{ $item->tamu->nama ?? 'Anonim' }}</h4>
                    ...
                    <div class="flex-1 space-y-4">
                        <div
                            class="relative p-4 bg-slate-50 dark:bg-slate-900/50 rounded-2xl border border-slate-100 dark:border-slate-800 italic text-slate-700 dark:text-slate-300 text-sm leading-relaxed">
                            <i data-lucide="quote"
                                class="absolute -top-3 -left-2 w-8 h-8 text-indigo-500/10 rotate-180"></i>
                            "{{ $item->ucapan }}"
                        </div>
                        ...
                        <div class="space-y-3 pl-4 border-l-2 border-indigo-100 dark:border-indigo-800">
                            @if ($item->balas)
                                <div
                                    class="bg-indigo-50/50 dark:bg-indigo-900/10 p-3 rounded-xl text-xs text-indigo-700 dark:text-indigo-300">
                            ...
                            <div class="flex gap-2">
                                <input type="text" wire:model.defer="balas.{{ $item->id }}"
                                    placeholder="Tanggapi ucapan ini..."
                                ...
                                <button wire:click="tanggapi({{ $item->id }})"
                    ...
                    <div class="flex-shrink-0 self-start">
                        <button
                            ...
                            class="p-2 text-rose-400 hover:text-rose-600 hover:bg-rose-50 dark:hover:bg-rose-900/20 rounded-xl transition-all">
        ...
        @empty
            <div
                ...
                <div
                    class="w-16 h-16 bg-slate-50 dark:bg-slate-900/50 rounded-full flex items-center justify-center mb-4">
