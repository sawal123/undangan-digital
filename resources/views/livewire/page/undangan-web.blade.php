    ...
    <header class="sticky top-0 z-30 border-b border-slate-200/80 bg-white/90 shadow-sm backdrop-blur-md">
        <div class="mx-auto flex max-w-7xl items-center justify-between px-4 py-3.5 sm:px-6 lg:px-8">
            ...
            <nav class="hidden items-center gap-8 text-sm font-medium text-slate-600 md:flex">
                <a href="{{ route('web') }}" wire:navigate class="font-semibold text-amber-600">Web</a>
            ...
            <a href="{{ route('login') }}" wire:navigate
                class="hidden rounded-full border border-amber-200 bg-amber-50/60 px-4 py-2 text-xs font-semibold text-amber-700 shadow-sm transition hover:bg-amber-100/80 md:inline-flex items-center gap-1.5">
                <i class="fas fa-headset text-amber-500"></i> Konsultasi Gratis
            </a>

            <button type="button" class="p-2 text-slate-600 md:hidden" id="webMenuButton" aria-label="Buka menu">
                <i class="fas fa-bars text-xl"></i>
            </button>
        </div>

        <!-- Mobile Menu -->
        <div id="webMobileMenu" class="hidden border-t border-slate-200/80 bg-white px-5 py-4 text-slate-700 md:hidden">
            <div class="flex flex-col gap-3 text-sm">
                <a href="{{ route('web') }}" wire:navigate class="flex items-center gap-3 py-1.5 font-semibold text-amber-600">
                    <i class="fas fa-globe w-5 text-amber-500"></i> Web
                </a>
                <a href="{{ route('cetak') }}" wire:navigate class="flex items-center gap-3 py-1.5 transition hover:text-amber-600">
                    <i class="fas fa-print w-5 text-slate-400"></i> Cetak
                </a>
                ...
                <a href="{{ route('login') }}" wire:navigate class="mt-2 flex items-center justify-center gap-2 rounded-xl bg-amber-50 py-2.5 text-center font-semibold text-amber-700">
                    <i class="fas fa-headset text-amber-500"></i> Konsultasi Gratis
                </a>
            </div>
        </div>
    ...
    <main class="py-10 sm:py-14">
        <div class="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
            ...
            <div class="mb-10 text-center max-w-3xl mx-auto">
                <div class="mb-3 inline-flex items-center gap-2 rounded-full border border-amber-200/80 bg-amber-50/80 px-3.5 py-1 text-xs font-semibold text-amber-700">
                    <i class="fas fa-globe text-amber-500 text-[11px]"></i> Katalog Undangan Web
                </div>
                ...
                <p class="mt-2.5 text-sm sm:text-base text-slate-500 leading-relaxed max-w-xl mx-auto">
                    Pilih desain digital yang responsif, modern, dan siap dibagikan secara instan ke seluruh tamu undangan Anda.
                </p>
            </div>

            <!-- Search Bar Section -->
            <div class="mb-10 mx-auto max-w-2xl">
                <div class="relative flex items-center rounded-2xl border border-slate-200 bg-white p-1.5 shadow-sm transition-all focus-within:border-amber-300 focus-within:ring-2 focus-within:ring-amber-100">
                    ...
                    <input
                        ...
                        placeholder="Cari tema undangan berdasarkan nama atau kategori..."
                    ...
                    <div wire:loading wire:target="search" class="absolute right-3.5">
                        <x-loading-spinner class="w-4 h-4 text-amber-600" />
                    </div>
            ...
            <div wire:loading.remove wire:target="search">
                @if ($undanganWeb->isEmpty())
                    ...
                    <div class="mx-auto max-w-md rounded-3xl border border-slate-200/80 bg-white p-8 text-center shadow-sm my-8">
                        <div class="mx-auto mb-4 flex h-14 w-14 items-center justify-center rounded-2xl bg-amber-50 text-amber-500">
                            <i class="fas fa-images text-2xl"></i>
                        </div>
                        <h3 class="text-lg font-bold text-slate-800">Tema Tidak Ditemukan</h3>
                        <p class="mt-1 text-sm text-slate-500">Coba gunakan kata kunci lain seperti nama atau kategori tema.</p>
                        @if (!empty($search))
                            <button
                                type="button"
                                wire:click="$set('search', '')"
                ...
                @else
                    <div class="grid gap-6 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">
                        @foreach ($undanganWeb as $item)
                            <article class="group relative flex flex-col overflow-hidden rounded-2xl border border-slate-200/80 bg-white shadow-sm transition-all duration-300 hover:-translate-y-1 hover:shadow-md hover:border-slate-300">
                                <!-- Thumbnail Container -->
                                <div class="relative aspect-[4/3] w-full overflow-hidden bg-stone-50/80">
                                    <a href="{{ url('demo/' . $item->demo) }}" target="_blank" rel="noopener" class="block h-full w-full focus:outline-none">
                                        <img
                                    ...
                                    <div class="absolute top-3 left-3 pointer-events-none">
                                        <span class="inline-flex items-center gap-1 rounded-full bg-white/95 px-2.5 py-1 text-[11px] font-semibold text-slate-700 shadow-sm border border-slate-200/60 backdrop-blur-md">
                                            <i class="fas fa-layer-group text-[10px] text-amber-500"></i> {{ $item->category->category ?? 'Digital' }}
                                        </span>
                                    </div>
                                </div>
                                ...
                                <div class="flex flex-1 flex-col p-4 sm:p-5 justify-between">
                                    <div>
                                        <h3 class="text-base font-bold text-slate-800 line-clamp-2 min-h-[3rem] group-hover:text-amber-600 transition-colors">
                                            {{ $item->nama }}
                                        </h3>
                                        <p class="mt-1 text-xs text-slate-500 flex items-center gap-1.5">
                                            <i class="fas fa-mobile-screen-button text-amber-500"></i> Responsif & Siap Pakai
                                        </p>
                                    </div>

                                    <!-- Action Button -->
                                    <a
                                        href="{{ url('demo/' . $item->demo) }}"
                                        target="_blank"
                                        rel="noopener"
                                        class="mt-4 inline-flex h-10 w-full items-center justify-center gap-2 rounded-xl border border-amber-200 bg-amber-50/70 px-4 text-sm font-semibold text-amber-700 transition hover:border-amber-300 hover:bg-amber-100/80 active:bg-amber-200/80"
                                    >
                                        <span>Preview Tema</span>
                                        <i class="fas fa-arrow-up-right-from-square text-xs transition-transform group-hover:translate-x-0.5 group-hover:-translate-y-0.5"></i>
                                    </a>
                                </div>
                            </article>
                        @endforeach
